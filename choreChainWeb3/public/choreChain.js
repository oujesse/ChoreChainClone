...
new Promise((resolve) => {
    ...
    instance = inst;
    addHouse(web3.eth.defaultAccount, instance, housemateAddresses, hID);
}).then(() => { //adds all housemates as users on the ChoreChain contract
    for(var i = 0; i < housemateAddresses.length; i++) {
        addUser(housemateAddresses[i], instance);
    }
}).then(() => { //everyone listed in the house agrees to the house
    for(var i = 0; i < housemateAddresses.length; i++) {
        agreeToHouse(housemateAddresses[i], instance, hID);
    }
}).then(() => { //initializes a list of chores
    return choreConstruct(web3.eth.defaultAccount, housemateAddresses, "Do the dishes", 100, web3.eth.defaultAccount);
}).then((cInst) => {
    chores.push(cInst);
}).then(() => { //initializes a list of chores
    ...
    for(var i = 0; i < chores.length; i++) {
        console.log("Chore " + i + " address: " + chores[i].options.address);
    }
}).then(() => { //everyone agrees these are vaild chores
    for(var j = 0; j < chores.length; j++){
        for(var i = 0; i < housemateAddresses.length; i++) {
            agreeToCreateChore(housemateAddresses[i], chores[j]);
        }
    }
}).then(() => {
    for(var j = 0; j < chores.length - 1; j++){
        for(var i = 0; i < housemateAddresses.length; i++) {
            vote(housemateAddresses[i], chores[j], false);
        }
    }
}).then(() => {
    for(var i = 0; i < chores.length - 1; i++) {
        judgeChore(web3.eth.defaultAccount, instance, hID, chores[i].options.address);
    }
}).then(() => {
    for(var i = 0; i < housemateAddresses.length; i++) {
        terminateHouse(housemateAddresses[i], instance, hID);
    }
}).then(() => {
    for(var i = 0; i < housemateAddresses.length; i++) {
        assessHouse(housemateAddresses[i], instance, hID);
    }
});
